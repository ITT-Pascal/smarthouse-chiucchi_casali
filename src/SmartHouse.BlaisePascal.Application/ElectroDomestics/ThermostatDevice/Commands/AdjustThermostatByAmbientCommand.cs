...
{
    public class AdjustThermostatByAmbientCommand
    {
        private readonly IThermostatRepository _repository;

        public AdjustThermostatByAmbientCommand(IThermostatRepository repository)
        {
            _repository = repository;
        }

        public void Execute(Guid id, int ambientTemperature)
        {
            var thermostat = _repository.GetById(id);
            if (thermostat != null)
            {
                thermostat.AdjustTemperatureByAmbientTemperature(ambientTemperature);
